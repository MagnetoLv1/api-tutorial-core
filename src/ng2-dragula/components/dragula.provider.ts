...
export class DragulaService {
  ...
  public setOptions(name: string, options: any): void {
    console.log('setOptions', name,options);
    let d = new Dragula(options);
    let bag = this.add(name,d.drake);
  ...
  private handleModels(name: string, drake: any): void {
    ...
    drake.on('drop', (dropElm: any, target: any, source: any) => {
      ...
      // console.log(sourceModel);
      if (target === source) {
        let down = dragIndex > dropIndex? 1:0;
        console.log(dragIndex , dropIndex, down)
        let dropModel =  sourceModel.splice(dragIndex, 1)[0];
        sourceModel.splice(dropIndex+down, 0, dropModel);
        console.log(sourceModel);
      } else {
